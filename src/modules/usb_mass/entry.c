...
static const struct guid block_guid = { 0x964e5b21, 0x6459, 0x11d2,
	{ 0x8e, 0x39, 0x00, 0xa0, 0xc9, 0x69, 0x72, 0x3b } };
static const struct guid block2_guid = { 0xa77b2472, 0xe282, 0x4e9f,
...
static struct cdk2_usb_mass_binding mass_binding;
static struct driver_binding binding;
static struct component_name component, component2;
static unload_fn *original_unload;
static CHAR16 driver_name[] = L"USB Mass Storage Driver";
static CHAR16 controller_name[] = L"USB Mass Storage Device";
...
static EFI_STATUS get_parent_path(void *controller, void **path, UINTN *size)
{
	UINT8 *bytes;
	UINTN offset = 0U;
	EFI_STATUS status = bs->open_protocol(controller, &path_guid, path,
		binding.handle, controller, 0x02U);
	...
	struct cdk2_usb_mass_child *child, void **handle)
{
	...
	(void)context;
	status = get_parent_path(controller, &parent, &size);
	if (EFI_ERROR(status))
		return status;
	...
	if (EFI_ERROR(status))
		return status;
	memcpy(path, parent, size); memcpy(path + size, &child->path,
		sizeof(child->path)); memcpy(path + size + sizeof(child->path), end,
		sizeof(end));
	child->device_path = path;
	*handle = NULL;
	status = bs->install_multiple(handle, &path_guid, path, &block_guid,
		&child->block.block, &block2_guid, &child->block.block2, NULL);
	if (EFI_ERROR(status)) {
	...
	struct cdk2_usb_mass_child *child, void *handle)
{
	...
	status = bs->uninstall_multiple(handle, &path_guid, child->device_path,
		&block_guid, &child->block.block, &block2_guid, &child->block.block2, NULL);
	if (!EFI_ERROR(status)) {
...
{ void *usb; (void)context; return bs->open_protocol(controller, &usb_io_guid,
	&usb, binding.handle, child, 0x08U); }
static EFI_STATUS unlink_child(void *context, void *controller, void *child)
{ (void)context; return bs->close_protocol(controller, &usb_io_guid,
	binding.handle, child); }
...
{ (void)driver; (void)remaining;
	return cdk2_usb_mass_binding_supported(&mass_binding, controller); }
static EFI_STATUS CDK2_MS_ABI start(struct driver_binding *driver,
	void *controller, void *remaining)
{ (void)driver; (void)remaining;
	return cdk2_usb_mass_binding_start(&mass_binding, controller); }
static EFI_STATUS CDK2_MS_ABI stop(struct driver_binding *driver,
	void *controller, UINTN count, void **children)
{ (void)driver; return cdk2_usb_mass_binding_stop(&mass_binding, controller,
	count, children); }
...
{ if (controller == NULL || language == NULL || name == NULL)
	...
	if ((this == &component && !language_equal(language, "eng")) ||
	    (this == &component2 && !language_equal(language, "en")))
		return EFI_UNSUPPORTED;
	for (UINTN index = 0U; index < mass_binding.count; index++)
		if (mass_binding.controllers[index]->handle == controller) {
			if (child != NULL) {
				UINTN lun = 0U;

				while (lun < mass_binding.controllers[index]->child_count &&
				       mass_binding.controllers[index]->children[lun].handle != child)
					lun++;
				if (lun == mass_binding.controllers[index]->child_count)
					return EFI_UNSUPPORTED;
			}
...
{
	while (mass_binding.count != 0U) {
		EFI_STATUS status = cdk2_usb_mass_binding_stop(&mass_binding,
			mass_binding.controllers[mass_binding.count - 1U]->handle, 0U, NULL);
		if (EFI_ERROR(status))
			return status;
	}
...
EFI_STATUS CDK2_MS_ABI cdk2_usb_mass_entry(void *image,
	struct system_table *system)
{
	struct cdk2_usb_mass_binding_services services = { .open_usb = open_usb,
		.close_usb = close_usb, .publish = publish, .remove = remove_child,
		.link = link_child, .unlink = unlink_child, .allocate = allocate,
		.release = release };
	EFI_STATUS status;
	...
	if (EFI_ERROR(status))
		return status;
	status = cdk2_usb_mass_binding_init(&mass_binding, &services);
	if (EFI_ERROR(status))
		return status;
