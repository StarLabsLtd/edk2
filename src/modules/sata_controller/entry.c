...
{ return (struct cdk2_ata_mode){ (UINT16)mode->pio.mode, (UINT16)mode->udma.mode,
	mode->pio.valid, mode->udma.valid }; }
...
{ struct cdk2_ata_mode mode; struct collective *value; EFI_STATUS status;
	...
	if (EFI_ERROR(status)) {
		(void)bs->free_pool(value);
		return status;
	}
	value->pio.valid = mode.pio_valid; value->pio.mode = mode.pio_mode;
	value->udma.valid = mode.udma_valid; value->udma.mode = mode.udma_mode;
	*result = value; return EFI_SUCCESS; }
...
{ struct context *ctx; struct cdk2_sata_geometry geometry; UINT8 base, sub;
	...
	status = bs->allocate_pool(4U, sizeof(*ctx), (void **)&ctx);
	if (EFI_ERROR(status))
		goto close;
	__builtin_memset(ctx, 0, sizeof(*ctx)); ctx->pci = pci; ctx->handle = handle;
	status = pci->attributes(pci, 0U, 0U, &ctx->original);
	if (EFI_ERROR(status))
		goto free;
	status = pci->attributes(pci, 4U, 0U, &supported_attributes);
	if (EFI_ERROR(status))
		goto free;
	...
	if (EFI_ERROR(status))
		goto free;
	ctx->changed = TRUE;
	status = read_class(pci, &base, &sub);
	if (EFI_ERROR(status))
		goto restore;
	if (sub == 6U) {
		status = pci->mem.read(pci, 2U, 5U, 0xcU, 1U, &pi);
		if (EFI_ERROR(status) || pi == 0U) {
			status = EFI_UNSUPPORTED;
			goto restore;
		}
		status = pci->mem.read(pci, 2U, 5U, 0U, 1U, &cap);
		if (EFI_ERROR(status))
			goto restore;
	}
	status = cdk2_sata_geometry(base, sub, cap, pi, &geometry);
	if (EFI_ERROR(status))
		goto restore;
	status = cdk2_sata_controller_init(&ctx->controller, &geometry);
	if (EFI_ERROR(status))
		goto restore;
...
restore:
	if (ctx->changed)
		(void)pci->attributes(pci, 1U, ctx->original, NULL);
...
{ struct ide_protocol *ide; struct context *ctx; EFI_STATUS status;
	...
	(void)children; (void)child_handles; status = bs->open_protocol(handle, &ide_guid,
		(void **)&ide, driver->handle, handle, 0x02U);
	if (EFI_ERROR(status))
		return EFI_UNSUPPORTED;
	ctx = from_ide(ide); owner = &managed;
	while (*owner != NULL && *owner != ctx)
		owner = &(*owner)->next;
	if (*owner != ctx)
		return EFIERR(15);
	status = bs->uninstall_multiple(handle, &ide_guid, ide, NULL);
	if (EFI_ERROR(status))
		return status;
	*owner = ctx->next;
	if (ctx->changed)
...
static EFI_STATUS CDK2_MS_ABI get_name(void *protocol, CHAR8 *language, CHAR16 **result)
{ if (language == NULL || result == NULL) return EFI_INVALID_PARAMETER;
	if ((protocol == &component && !language_equal(language, "eng")) ||
	    (protocol == &component2 && !language_equal(language, "en")))
		return EFI_UNSUPPORTED;
	*result = name; return EFI_SUCCESS; }
static EFI_STATUS CDK2_MS_ABI get_controller_name(void *protocol, void *controller,
	void *child, CHAR8 *language, CHAR16 **result)
{
	...
	if (controller == NULL || language == NULL || result == NULL)
		return EFI_INVALID_PARAMETER;
	if (child != NULL ||
	    ((protocol == &component && !language_equal(language, "eng")) ||
	     (protocol == &component2 && !language_equal(language, "en"))))
		return EFI_UNSUPPORTED;
	{
		struct context *owner = managed;
		while (owner != NULL && owner->handle != controller)
			owner = owner->next;
		if (owner == NULL)
			return EFI_UNSUPPORTED;
	}
