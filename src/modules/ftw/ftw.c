...
EFI_STATUS cdk2_ftw_initialize(struct cdk2_ftw *ftw)
{
	...
	if (status == EFI_NOT_FOUND) {
		zero(&ftw->journal, sizeof(ftw->journal));
		ftw->journal.magic = CDK2_FTW_MAGIC;
		ftw->journal.phase = CDK2_FTW_EMPTY;
		return persist(ftw);
	}
	if (EFI_ERROR(status))
		return status;
...
{
	if (EFI_ERROR(valid_context(ftw)) || !caller || !writes)
		return EFI_INVALID_PARAMETER;
	if (private_size > CDK2_FTW_MAX_PRIVATE || writes > CDK2_FTW_MAX_WRITES)
		return EFI_BUFFER_TOO_SMALL;
	if (ftw->journal.phase == CDK2_FTW_ALLOCATED ||
	    ftw->journal.phase == CDK2_FTW_SPARE_COMPLETE)
		return EFI_ACCESS_DENIED;
	zero(&ftw->journal, sizeof(ftw->journal));
	ftw->journal.magic = CDK2_FTW_MAGIC;
	ftw->journal.caller_id = *caller;
	ftw->journal.private_size = private_size;
	ftw->journal.write_count = writes;
	ftw->journal.phase = CDK2_FTW_ALLOCATED;
	return persist(ftw);
}
void cdk2_ftw_set_relative_offset(struct cdk2_ftw *ftw, INT64 relative_offset)
{
	if (ftw)
		ftw->relative_offset = relative_offset;
}
			  ...
			  UINTN length, const void *private_data, const void *buffer)
{
	...
	if (EFI_ERROR(valid_context(ftw)) || (!buffer && length) ||
	    (!private_data && ftw->journal.private_size))
		return EFI_INVALID_PARAMETER;
	...
	status = ftw->ops->spare_erase(ftw->context, 0);
	if (EFI_ERROR(status))
		return EFI_ABORTED;
	status = ftw->ops->spare_write(ftw->context, 0, ftw->scratch, ftw->block_size);
	if (EFI_ERROR(status))
		return EFI_ABORTED;
	record->phase = CDK2_FTW_SPARE_COMPLETE;
	ftw->journal.phase = CDK2_FTW_SPARE_COMPLETE;
	status = persist(ftw);
	if (EFI_ERROR(status))
		return EFI_ABORTED;
...
{
	struct cdk2_ftw_record *record;
	if (EFI_ERROR(valid_context(ftw)))
		return EFI_INVALID_PARAMETER;
	if (ftw->journal.next_write >= ftw->journal.write_count)
		return EFI_ACCESS_DENIED;
	record = &ftw->journal.records[ftw->journal.next_write];
	if (record->phase != CDK2_FTW_SPARE_COMPLETE)
		return EFI_ACCESS_DENIED;
	return finish_from_spare(ftw, record);
}
EFI_STATUS cdk2_ftw_abort(struct cdk2_ftw *ftw)
{
	...
	ftw->journal.phase = CDK2_FTW_ABORTED;
	return persist(ftw);
}
				   ...
				   void *private_data, BOOLEAN *complete)
{
	...
	if (!ftw || !caller || !lba || !offset || !length || !private_size || !complete)
		return EFI_INVALID_PARAMETER;
	if (!ftw->journal.write_count || ftw->journal.phase == CDK2_FTW_EMPTY)
		return EFI_NOT_FOUND;
	index = ftw->journal.next_write < ftw->journal.write_count ? ftw->journal.next_write :
		ftw->journal.write_count - 1;
	record = &ftw->journal.records[index];
	if (record->phase == CDK2_FTW_EMPTY && index > 0)
		record = &ftw->journal.records[--index];
	if (*private_size < record->private_size) {
		*private_size = record->private_size;
		return EFI_BUFFER_TOO_SMALL;
	}
	if (record->private_size && !private_data)
		return EFI_INVALID_PARAMETER;
	*caller = ftw->journal.caller_id;
	*lba = record->lba;
