...
{
	return entry.boot->slots_before_locate_protocol[(offset - 24U) / sizeof(void *)];
}
...
static const SMMSTORE_INFO *find_smmstore(const struct cdk2_ftw_system_table_view *system)
{
	...
	while (walked + sizeof(*hob) <= MAX_HOB_LIST_SIZE) {
		...
		walked += hob->length; hob = (const void *)(list + walked);
	}
	return NULL;
}
...
{
	UINTN bytes = size; EFI_STATUS status;
	(void)context;
...
{ (void)context; (void)volume; return FALSE; }
...
static EFI_STATUS workspace_load(void)
{
	...
	if (!EFI_ERROR(status)) entry.workspace_loaded = TRUE;
	return status;
}
static EFI_STATUS journal_read(void *context, struct cdk2_ftw_journal *journal)
{
	UINTN index; BOOLEAN erased = TRUE; EFI_STATUS status;
	(void)context;
	if (!entry.workspace_loaded) { status = workspace_load(); if (EFI_ERROR(status)) return status; }
	for (index = 0; index < entry.geometry.working_size; index++)
		if (entry.workspace[index] != 0xffU) { erased = FALSE; break; }
	if (erased) return EFI_NOT_FOUND;
...
{
	UINTN index, bytes; BOOLEAN erase = FALSE; EFI_STATUS status;
	(void)context;
	if (!entry.workspace_loaded) { status = workspace_load(); if (EFI_ERROR(status)) return status; }
	for (index = 0; index < entry.geometry.working_size; index++)
	...
	if (!EFI_ERROR(status)) status = cdk2_ftw_pi_encode(entry.workspace,
...
{ (void)self; if (size == NULL) return EFI_INVALID_PARAMETER; *size = entry.adapter.core.block_size; return EFI_SUCCESS; }
static EFI_STATUS CDK2_MS_ABI protocol_allocate(void *self, EFI_GUID *caller,
	UINTN private_size, UINTN writes)
{ (void)self; return cdk2_ftw_allocate(&entry.adapter.core, caller, private_size, writes); }
	...
	UINTN length, void *private_data, void *handle, void *buffer)
{
	...
	struct cdk2_fvb_protocol_view *target; UINT64 base; UINTN block, count;
	EFI_STATUS status; (void)self;
	if (handle_protocol == NULL || handle == NULL) return EFI_INVALID_PARAMETER;
	status = handle_protocol(handle, &fvb_guid, (void **)&target);
	if (EFI_ERROR(status)) return status;
	status = target->get_physical_address(target, &base);
	if (!EFI_ERROR(status)) status = target->get_block_size(target, lba, &block, &count);
	if (EFI_ERROR(status)) return status;
	if (!block || lba > (MAX_UINT64 - base) / block) return EFI_BAD_BUFFER_SIZE;
...
{ (void)self; (void)handle; return cdk2_ftw_restart(&entry.adapter.core); }
static EFI_STATUS CDK2_MS_ABI protocol_abort(void *self)
{ (void)self; return cdk2_ftw_abort(&entry.adapter.core); }
...
{ (void)self; return cdk2_ftw_get_last_write(&entry.adapter.core, caller, lba, offset,
	length, private_size, private_data, complete); }

static void release_buffers(void)
{
	free_pool_fn *free_pool = (void *)boot_slot(72U);
	if (free_pool != NULL && entry.workspace != NULL) (void)free_pool(entry.workspace);
	if (free_pool != NULL && entry.scratch != NULL) (void)free_pool(entry.scratch);
	entry.workspace = NULL; entry.scratch = NULL;
}
...
static EFI_STATUS discover_and_install(void)
{
	...
	free_pool_fn *free_pool = (void *)boot_slot(72U);
	void **handles = NULL, *runtime_arch; UINTN count = 0, index;
	EFI_STATUS status = EFI_NOT_FOUND;
	if (entry.installed) return EFI_ALREADY_STARTED;
	if (!locate_handles || !handle_protocol || !locate_protocol || !install_protocol ||
	    !allocate_pool || !free_pool) return EFI_UNSUPPORTED;
	status = locate_protocol(&runtime_arch_guid, NULL, &runtime_arch);
	if (EFI_ERROR(status)) return status == EFI_NOT_FOUND ? EFI_NOT_READY : status;
	status = locate_handles(BY_PROTOCOL, &fvb_guid, NULL, &count, &handles);
	if (EFI_ERROR(status)) return status;
	for (index = 0; index < count; index++) {
		struct cdk2_fvb_protocol_view *candidate; UINT64 base;
		...
		if (base <= entry.geometry.working_base &&
			...
			entry.fvb = candidate; entry.fvb_base = base; break;
		}
	}
	if (handles != NULL) (void)free_pool(handles);
	if (entry.fvb == NULL) return EFI_NOT_FOUND;
	status = allocate_pool(POOL_BOOT_SERVICES_DATA, entry.geometry.spare_size,
		(void **)&entry.scratch);
	if (!EFI_ERROR(status)) status = allocate_pool(POOL_BOOT_SERVICES_DATA,
		entry.geometry.working_size, (void **)&entry.workspace);
	if (EFI_ERROR(status)) { release_buffers(); return status; }
	entry.adapter = (struct cdk2_ftw_fvb) { .ops = &fvb_ops, .context = &entry,
	...
	if (EFI_ERROR(status)) { release_buffers(); return status; }
	entry.protocol = (struct cdk2_ftw_protocol_view) { protocol_max, protocol_allocate,
...
static void CDK2_MS_ABI fvb_available(void *event, void *context)
{
	...
	if (!EFI_ERROR(discover_and_install()) && close_event != NULL) {
		(void)close_event(entry.notify_event); entry.notify_event = NULL;
	}
}

EFI_STATUS CDK2_MS_ABI cdk2_ftw_entry(void *image, struct cdk2_ftw_system_table_view *system)
{
	const SMMSTORE_INFO *info; create_event_fn *create_event;
	register_notify_fn *register_notify; EFI_STATUS status;
	if (system == NULL || system->boot == NULL ||
	    (system->table_count != 0U && system->tables == NULL)) return EFI_INVALID_PARAMETER;
	entry.boot = system->boot; entry.image = image;
	...
	create_event = (void *)boot_slot(80U); register_notify = (void *)boot_slot(168U);
	if (create_event == NULL || register_notify == NULL) return EFI_UNSUPPORTED;
	...
	if (EFI_ERROR(status)) {
		close_event_fn *close_event = (void *)boot_slot(112U);
		if (close_event != NULL) (void)close_event(entry.notify_event);
...
{
	entry = (struct ftw_entry_context) { 0 };
}
