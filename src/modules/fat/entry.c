...
typedef EFI_STATUS CDK2_MS_ABI open_fn(void *, const EFI_GUID *, void **, void *, void *,
				       UINT32);
...
struct boot_view {
	...
	void *check_event, *install_protocol, *reinstall_protocol, *uninstall_protocol,
	    *handle_protocol, *reserved, *register_protocol_notify, *locate_handle,
	    *locate_device_path, *install_configuration_table, *load_image, *start_image, *exit,
	    *unload_image, *exit_boot_services, *get_next_monotonic_count, *stall,
	    *set_watchdog_timer, *connect_controller, *disconnect_controller;
...
typedef EFI_STATUS CDK2_MS_ABI name_fn(struct component_view *component,
	char *language, uint16_t **name);
typedef EFI_STATUS CDK2_MS_ABI controller_name_fn(struct component_view *component,
	void *controller, void *child, char *language, uint16_t **name);
struct component_view {
	name_fn *driver_name;
...
struct entry_context {
	...
	struct driver_view driver;
	struct component_view component, component2;
};
...
static const EFI_GUID driver_guid = {
	0x18a031abU, 0xb443U, 0x4d1aU, {0xa5U, 0xc0U, 0x0cU, 0x09U, 0x26U, 0x1eU, 0x9fU, 0x71U}};
static const EFI_GUID component_guid = {
	0x107a772cU, 0xd5e1U, 0x11d4U, {0x9aU, 0x46U, 0x00U, 0x90U, 0x27U, 0x3fU, 0xc1U, 0x4dU}};
static const EFI_GUID component2_guid = {
	0x6a7a5cffU, 0xe8d9U, 0x4f70U, {0xbaU, 0xdaU, 0x75U, 0xabU, 0x30U, 0x25U, 0xceU, 0x14U}};
static const EFI_GUID collation2_guid = {
	0xa4c751fcU, 0x23aeU, 0x4c3eU, {0x92U, 0xe9U, 0x49U, 0x64U, 0xcfU, 0x63U, 0xf3U, 0x49U}};
static const EFI_GUID collation_guid = {
	0x1d85cd7fU, 0xf43dU, 0x11d2U, {0x9aU, 0x0cU, 0x00U, 0x90U, 0x27U, 0x3fU, 0xc1U, 0x4dU}};
...
{
	return ((struct entry_context *)c)
	    ->boot->install_multiple(&controller, g, interface, NULL);
}
static EFI_STATUS op_unpublish(void *c, void *controller, const EFI_GUID *g, void *interface)
{
	return ((struct entry_context *)c)
	    ->boot->uninstall_multiple(controller, g, interface, NULL);
}
...
{
	struct entry_context *e = c;
	struct queued_call *call;
	EFI_STATUS status;
	if (function == NULL || cookie == NULL || e->boot->create_event == NULL ||
	    e->boot->set_timer == NULL || e->boot->close_event == NULL)
		return EFI_UNSUPPORTED;
	status = op_allocate(e, sizeof(*call), (void **)&call);
	if (EFI_ERROR(status))
		return status;
	*call = (struct queued_call){e, function, context, NULL};
	status = e->boot->create_event(0x80000200U, 8U, queued_notify, call, &call->event);
	if (!EFI_ERROR(status))
		status = e->boot->set_timer(call->event, 2U, 10000U);
	if (EFI_ERROR(status)) {
		if (call->event != NULL)
...
{
	struct entry_context *e = c;
	struct queued_call *call = cookie;
	if (call == NULL)
		return;
...
static EFI_STATUS op_create_event(void *c, void(CDK2_MS_ABI *notify)(void *, void *),
				  void *context, void **event)
{
	struct entry_context *e = c;
	return e->boot->create_event == NULL
		   ? EFI_UNSUPPORTED
...
{
	struct entry_context *e = c;
	return e->boot->close_event == NULL ? EFI_UNSUPPORTED : e->boot->close_event(event);
}
...
static EFI_STATUS CDK2_MS_ABI supported(struct driver_view *d, void *controller,
					void *remaining)
{
	...
	EFI_STATUS status;
	(void)remaining;
	status = op_open(e, controller, &cdk2_fat_block_io_guid, &interface);
	if (EFI_ERROR(status))
		return status;
	status = op_open(e, controller, &cdk2_fat_disk_io_guid, &interface);
	if (!EFI_ERROR(status))
		(void)op_close(e, controller, &cdk2_fat_disk_io_guid);
	(void)op_close(e, controller, &cdk2_fat_block_io_guid);
	return status;
}
static EFI_STATUS CDK2_MS_ABI start(struct driver_view *d, void *controller, void *remaining)
{
	...
	if (e->boot->locate_protocol == NULL)
		return EFI_UNSUPPORTED;
	status =
	    e->boot->locate_protocol(&collation2_guid, NULL, (void **)&e->binding.collation);
	if (EFI_ERROR(status))
...
static EFI_STATUS CDK2_MS_ABI stop(struct driver_view *d, void *controller, UINTN children,
				   void **buffer)
{
	(void)buffer;
	if (children != 0U)
...
static EFI_STATUS CDK2_MS_ABI driver_name(struct component_view *c, CHAR8 *language,
					  CHAR16 **name)
{
...
EFI_STATUS CDK2_MS_ABI cdk2_fat_entry(void *image, void *table)
{
	static const struct cdk2_fat_binding_ops ops = {
	    op_open,   op_close, op_publish, op_unpublish,    op_allocate,    op_release,
	    op_signal, op_queue, op_drain,   op_create_event, op_close_event, op_wait_event};
	...
	entry.image = image;
	entry.binding.ops = &ops;
	entry.binding.context = &entry;
	...
	status = entry.boot->install_multiple(&image, &driver_guid, &entry.driver,
					      &component_guid, &entry.component,
