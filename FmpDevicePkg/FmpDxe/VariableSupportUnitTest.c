  ...
  )
{
  ...
  ZeroMem (&mPrivate, sizeof (mPrivate));
  mPrivate.FmpStateVariableName = L"FmpState";
  UT_ASSERT_EQUAL (GetLastAttemptVersionFromVariable (&mPrivate), 9);
  UT_ASSERT_EQUAL (GetLastAttemptStatusFromVariable (&mPrivate), LAST_ATTEMPT_STATUS_ERROR_UNSUCCESSFUL);
