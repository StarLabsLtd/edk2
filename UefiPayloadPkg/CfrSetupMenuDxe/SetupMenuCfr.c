...
//
// Minimal copy of OPAL_DEVICE_LOCKBOX_DATA for parsing status, without pulling
// in SecurityPkg headers.
//
...
} OPAL_DEVICE_LOCKBOX_DATA_FIXED;
...
STATIC GUID  mOpalDeviceLockBoxGuid = {
  ...
  )
{
  ...
  *SupportedDisks = 0;
  ...
  FreePool (Results);
  if (EFI_ERROR (Status) || (BlockSize < sizeof (Block))) {
    return FALSE;
  }
  ...
  )
{
  ...
  *BufferSize = 0;
  ...
  Size   = sizeof (Dummy);
  Status = RestoreLockBox (&mOpalDeviceLockBoxGuid, &Dummy, &Size);
  ...
  Status = RestoreLockBox (&mOpalDeviceLockBoxGuid, Data, &Size);
  if (EFI_ERROR (Status)) {
    FreePool (Data);
    return Status;
  }
  ...
  )
{
  ...
  if ((Buffer == NULL) || (BufferSize < sizeof (OPAL_DEVICE_LOCKBOX_DATA_FIXED))) {
    return FALSE;
  }
  ...
  while (Remaining >= sizeof (OPAL_DEVICE_LOCKBOX_DATA_FIXED)) {
    CONST OPAL_DEVICE_LOCKBOX_DATA_FIXED  *Entry;
    UINT32                                EntrySize;

    Entry = (CONST OPAL_DEVICE_LOCKBOX_DATA_FIXED *)(CONST VOID *)Ptr;
    EntrySize = Entry->Length;

    if ((EntrySize < sizeof (OPAL_DEVICE_LOCKBOX_DATA_FIXED)) || (EntrySize > Remaining)) {
      break;
    }
