  ...
  )
{
  ...
  FreePool (Results);
  //
  // ConfigToBlock returns the largest byte index (0-based), not the byte count.
  // For 6 bytes (indices 0-5) it returns 5.
  //
  ...
  )
{
  ...
  *SupportedDisks = 0;
  ...
  if (TryGetOpalSupportedDisksFromExtractConfig (ConfigAccess, DriverHandle, SupportedDisks)) {
    return TRUE;
  }

  return TryGetOpalSupportedDisksFromBrowserData (SupportedDisks);
}
  ...
  )
{
  ...
  //
  // If the OpalPassword driver isn't present, treat as not available.
  //
  HiiHandles = HiiGetHiiHandles (&mOpalPackageListGuid);
  if (HiiHandles == NULL) {
  ...
  //
  // We cannot accurately determine if a password is set (LockBox Restore returns
  // NOT_FOUND on coreboot payloads). Show "Setup" when Opal disks are present
  // so the user can open the form; otherwise "Not Available".
  //
  return (SupportedDisksValid && (SupportedDisks != 0)) ? L"Setup" : L"Not Available";
}
