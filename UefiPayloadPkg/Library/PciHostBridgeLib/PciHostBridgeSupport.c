  ...
  )
{
  ...
  for (Index = 0; Index < NumberOfDescriptors; Index++) {
    ...
    if (Map[Index].GcdMemoryType == EfiGcdMemoryTypeReserved) {
      DEBUG ((
        DEBUG_ERROR,
        "%a: PCI aperture overlaps reserved memory region\n",
        __func__
        ));
      DEBUG ((
        DEBUG_ERROR,
        "%a: Please consider fixing EDK2 or your firmware\n",
        __func__
        ));
  ...
  for (Offset = BarOffsetBase; Offset < BarOffsetEnd; Offset += sizeof (UINT32)) {
    ...
    if ((Value & BIT0) == BIT0) {
      ...
      if ((Command & EFI_PCI_COMMAND_MEMORY_SPACE) != 0) {
        ...
        Base          = OriginalValue & Mask;
        Length        = Value & Mask;
        Prefetchable  = ((OriginalValue & BIT3) != 0);
        ...
        if ((Value & (BIT1 | BIT2)) == 0) {
          ...
          MemAperture = Prefetchable ? PMem : Mem;
        } else {
          //
          // 64bit
          //
          ...
          if (Base < BASE_4GB) {
            ...
            MemAperture = Prefetchable ? PMemAbove4G : MemAbove4G;
          }
        }
        ...
        if ((Base > 0) && (Base < Limit)) {
          ...
          // this would cause the aperture to cover the MMCONF window, which
          ...
          if (EFI_ERROR (Status)) {
            DEBUG ((
              DEBUG_ERROR,
              "%a: Not adding PCI %x:%x.%x BAR@%d %x:%x to aperture\n",
              __func__,
              Bus,
              Device,
              Function,
  ...
  *NumberOfRootBridges = 0;
  ...
  for (PrimaryBus = 0; PrimaryBus <= PCI_MAX_BUS; PrimaryBus = SubBus + 1) {
    ...
    if (NumberOfDevices > 0) {
      if (PMem.Base == MAX_UINT64) {
        PMem = mNonExistAperture;
      }

      if (PMemAbove4G.Base == MAX_UINT64) {
        PMemAbove4G = mNonExistAperture;
      }
