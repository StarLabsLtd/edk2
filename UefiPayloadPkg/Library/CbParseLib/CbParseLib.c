...
/**
  ...
  The numeric values must stay in sync with the E820_* definitions
  in UefiPayloadEntry.h.
  ...
  )
{
  switch (CbType) {
    ...
    //
    // Vendor-reserved and table ranges are still DRAM from coreboot's
    // point of view and must not be treated as "disabled" memory.
    // Describe them as reserved memory to the E820 consumer.
    //
