/** @file
  ...
  EDK II HOB construction for the cdk2 entry path.
  ...
  )
{
      ...
      ((MemoryMapEntry->Base + MemoryMapEntry->Size) >= PcdGet32 (PcdPayloadFdMemBase)))
  {
    ...
    Status                   = Cdk2EfiFindFreeMemForHobCallback (&MemoryMapEntrySplit, Params);
    if (EFI_ERROR (Status)) {
      return Status;
    }
  ...
  )
{
  ...
  Status = ParseMemoryInfo (Cdk2EfiFindToludCallback, NULL);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Status = ParseMemoryInfo (Cdk2EfiMemInfoCallback, NULL);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Status = ParseMemoryInfo (Cdk2EfiMemInfoCallbackMmio, AcpiBoardInfo);
  if (EFI_ERROR (Status)) {
    return Status;
  }
