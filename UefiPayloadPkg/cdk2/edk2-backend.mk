...
CDK2_BOOL_DEFINES := \
  ...
  SECURE_BOOT_ASSUME_SETUP_PHYSICAL_PRESENCE:CDK2_SECURE_BOOT_USER_PHYSICAL_PRESENCE \
  EVIL_MAID_PROTECTION:CDK2_ANTI_TAMPER_BOOT \
  EVIL_MAID_REQUIRE_TPM_PPI:CDK2_ANTI_TAMPER_REQUIRE_TPM_PPI \
  EVIL_MAID_SHUTDOWN_ON_FAILURE:CDK2_ANTI_TAMPER_SHUTDOWN_ON_FAILURE \
  SETUP_UI:SETUP_UI_ENABLE \
  CONNECT_ALL_DEVICES:CONNECT_ALL_DEVICES \
  LINUX_EFI_APPLICATION:LINUX_EFI_APPLICATION_BOOT \
  TPM12:TPM1_ENABLE \
  TPM2:TPM2_ENABLE \
...
ifeq ($(CONFIG_CDK2_LINUX_EFI_APPLICATION),y)
ifneq ($(findstring ',$(CONFIG_CDK2_LINUX_EFI_APPLICATION_PATH)),)
$(error CONFIG_CDK2_LINUX_EFI_APPLICATION_PATH must not contain apostrophes)
endif
ifneq ($(findstring ',$(CONFIG_CDK2_LINUX_EFI_APPLICATION_DESCRIPTION)),)
$(error CONFIG_CDK2_LINUX_EFI_APPLICATION_DESCRIPTION must not contain apostrophes)
endif
CDK2_DEFINES += -D 'LINUX_EFI_APPLICATION_PATH=L$(CONFIG_CDK2_LINUX_EFI_APPLICATION_PATH)'
CDK2_DEFINES += -D 'LINUX_EFI_APPLICATION_DESCRIPTION=L$(CONFIG_CDK2_LINUX_EFI_APPLICATION_DESCRIPTION)'
endif
...
if test "$(CDK2_EFFECTIVE_EVIL_MAID_PROTECTION)" = "y"; then \
  test "$(CDK2_EFFECTIVE_SECURE_BOOT)" = "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION requires CONFIG_CDK2_SECURE_BOOT=y" >&2; exit 1; }; \
  test "$(CDK2_EFFECTIVE_TPM2)" = "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION requires CONFIG_CDK2_TPM2=y" >&2; exit 1; }; \
  test "$(CDK2_EFFECTIVE_TPM12)" != "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION requires TPM2-only; disable CONFIG_CDK2_TPM12" >&2; exit 1; }; \
  test "$(CDK2_EFFECTIVE_SMMSTORE)" = "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION requires CONFIG_CDK2_SMMSTORE=y" >&2; exit 1; }; \
  test "$(CDK2_EFFECTIVE_SHELL)" != "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION does not allow the built-in UEFI shell" >&2; exit 1; }; \
  test "$(CDK2_EFFECTIVE_SECURE_BOOT_ASSUME_SETUP_PHYSICAL_PRESENCE)" != "y" || { echo "CONFIG_CDK2_EVIL_MAID_PROTECTION does not allow setup to assert Secure Boot physical presence" >&2; exit 1; }; \
fi
