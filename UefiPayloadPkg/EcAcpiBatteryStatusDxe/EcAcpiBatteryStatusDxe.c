...
/**
  Get battery critical state.

  @param[in]  This             Pointer to the EFI_BATTERY_STATUS_PROTOCOL instance.
  @param[out] BatteryCritical  TRUE if battery is critical, FALSE otherwise.

  @retval EFI_SUCCESS            Battery critical state retrieved successfully.
  @retval EFI_DEVICE_ERROR       Communication error.
  @retval EFI_UNSUPPORTED        Battery critical state is not available or not supported.
  @retval EFI_INVALID_PARAMETER  One or more parameters are invalid.
...
EFI_STATUS
EFIAPI
EcAcpiBatteryGetBatteryCritical (
  IN  EFI_BATTERY_STATUS_PROTOCOL  *This,
  ...
  )
{
  ...
  if ((This == NULL) || (BatteryCritical == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  *BatteryCritical = FALSE;
  ...
  if ((mEcBatteryPrivate == NULL) || !mEcBatteryPrivate->EcPresent ||
      (mEcBatteryPrivate->Profile == NULL) ||
      (mEcBatteryPrivate->Profile->GetBatteryCritical == NULL))
  {
    return EFI_UNSUPPORTED;
  }

  Status = mEcBatteryPrivate->Profile->GetBatteryCritical (BatteryCritical);
  if (EFI_ERROR (Status)) {
    *BatteryCritical = FALSE;
  }
  ...
  )
{
  ...
  mEcBatteryPrivate->Protocol.GetBatteryCritical = EcAcpiBatteryGetBatteryCritical;
