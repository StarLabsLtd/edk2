  ...
  )
{
  ...
  while (!EFI_ERROR (Status)) {
    if (Record->Type == SMBIOS_TYPE_BIOS_INFORMATION) {
      ...
      UnicodeSPrint(NewString, 200 * sizeof(CHAR16), L"%-19s%8s", L"coreboot version:", BiosVersion);
      ...
      if (EcMajorVersion != 0x00 || EcMinorVersion != 0x00) {
        ...
        UnicodeSPrint(NewString, 100 * sizeof(CHAR16), L"%-16s%8s", L"EC version:", EcVersion);
      } else {
    ...
    if ((Record->Type == SMBIOS_TYPE_PROCESSOR_INFORMATION) && !FoundCpu) {
        ...
        if ((Type4Record->Status & SMBIOS_TYPE4_CPU_SOCKET_POPULATED) == SMBIOS_TYPE4_CPU_SOCKET_POPULATED) {
            ...
            UnicodeSPrint(PrependedNewString, 100 * sizeof(CHAR16), L"%-19s%s", L"Processor:", NewString);
  ...
  UiCustomizeFrontPageBanner (3, FALSE, &NewString);
  CHAR16 *TmpBuffer = AllocateZeroPool (0x60);
  UnicodeSPrint (TmpBuffer, 0x60 * sizeof (CHAR16), L"%-19s%s", L"Memory: ", NewString);
