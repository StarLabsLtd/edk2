...
{
	static const uint8_t guid[16] = { 0x06, 0x39, 0x86, 0x9e, 0x0f, 0xa4,
		0x75, 0x48, 0x97, 0x7f, 0x5b, 0x93, 0xff, 0x23, 0x7f, 0xc6 };
	static const uint8_t types[] = { 0x10, 0x15, 0x14 };
	static const uint8_t ui[] = { 'T', 0, 'e', 0, 'r', 0, 'm', 0, 'i', 0,
		'n', 0, 'a', 0, 'l', 0, 'D', 0, 'x', 0, 'e', 0, 0, 0 };
	...
	if (argc != 2 || (file = fopen(argv[1], "rb")) == NULL)
		return 1;
	if (fseek(file, 0, SEEK_END) != 0 || (length = ftell(file)) != 0xb046L ||
	...
	if (memcmp(data, guid, sizeof(guid)) != 0 || (sum & 0xffU) != 0U ||
	    data[17] != 0xaaU || data[18] != 7U || size24(data + 20) != 0xb046U ||
	    data[23] != 0xf8U) {
		free(data);
		return 1;
	}
	for (unsigned int index = 0U; index < sizeof(types); index++) {
		...
		if ((index == 0U && (section_size != 0xb004U ||
