...
int main(int argc, char **argv)
{
	...
	static const uint8_t ui[] = { 'S', 0, 'i', 0, 'o', 0, 'B', 0, 'u', 0,
		's', 0, 'D', 0, 'x', 0, 'e', 0, 0, 0 };
	...
	if (argc != 2 || (file = fopen(argv[1], "rb")) == NULL)
		return 1;
	if (fseek(file, 0, SEEK_END) != 0 || (length = ftell(file)) != 0x6042L ||
	...
	if (memcmp(data, guid, sizeof(guid)) != 0 || (sum & 0xffU) != 0U ||
	    data[17] != 0xaaU || data[18] != 7U || size24(data + 20) != 0x6042U ||
	    data[23] != 0xf8U) {
		free(data);
		return 1;
	}
	for (unsigned int index = 0U; index < sizeof(types); index++) {
		...
		if ((index == 0U && (section_size != 0x6004U ||
