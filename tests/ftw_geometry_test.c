...
int main(void)
{
	...
	info.mmio_address = 0; assert(cdk2_ftw_geometry_from_smmstore(&info, &g) == EFI_NOT_FOUND);
	info.mmio_address = 1; info.num_blocks = 7;
	assert(cdk2_ftw_geometry_from_smmstore(&info, &g) == EFI_NOT_FOUND);
	info.num_blocks = 8; info.block_size = 0;
	assert(cdk2_ftw_geometry_from_smmstore(&info, &g) == EFI_NOT_FOUND);
	return 0;
}
