...
{ UINT8 *id = packet->in_data; (void)p; (void)port; (void)device; (void)event;
	...
	put16(id + 106 * 2, 0x4000); put32(id + 100 * 2, 1000); return EFI_SUCCESS; }
...
{ struct fixture *f = context; (void)child; (void)packet; f->execute++; return EFI_SUCCESS; }
...
{
	memset(f, 0, sizeof(*f)); active = f; f->binding = binding;
...
int main(void)
{
	...
	CHECK(binding.controllers[1]->children[0]->block.block2.read_blocks(
		...
		sizeof(buffer), buffer) == EFI_SUCCESS && f.defers == 1 && f.execute == 0);
	f.reentrant_stop = 1;
	...
	CHECK(cdk2_ata_bus_binding_stop(&binding, (void *)2, 1, targets) == EFI_DEVICE_ERROR &&
		binding.controllers[1]->child_count == 2 && f.execute == 1 && f.signals == 1 &&
