...
struct fixture {
	...
	unsigned int opens, closes, publishes, unpublishes, allocations, releases;
	unsigned int fail_open, fail_publish, fail_unpublish, fail_close;
	unsigned int signals;
	unsigned int have_disk2;
};
static struct fixture *active;
static void (*queued_function)(void *);
static void *queued_context;
static void (CDK2_MS_ABI * disk_notify)(void *, void *);
static void *disk_context;
static void (CDK2_MS_ABI * event_notify[32])(void *, void *);
static void *event_context[32];
static unsigned int event_count;
static struct cdk2_disk_io2_token *disk_token;
static UINT64 disk2_offset;
static UINTN disk2_size;
static void *disk2_buffer;
static unsigned int disk2_calls, disk2_operation, disk2_writes;
...
struct test_file_info {
	...
	struct test_time create, access, modify;
	UINT64 attribute;
	CHAR16 file_name[2];
};
...
{
	(void)self;
	while (*left == *right && *left != 0U) {
		left++;
		right++;
	}
	return *left - *right;
}
static VOID CDK2_MS_ABI upper(struct cdk2_unicode_collation *self, uint16_t *text)
{
	(void)self;
	(void)text;
}
static BOOLEAN CDK2_MS_ABI to_fat(struct cdk2_unicode_collation *self, uint16_t *text, UINTN size,
				  char *fat)
{
	(void)self;
	(void)text;
...
static void format(struct fixture *f, unsigned int id)
{
	uint8_t *b = f->image[id];
	memset(b, 0, sizeof(f->image[id]));
	put16(b + 11U, 512U);
	b[13U] = 1U;
	put16(b + 14U, 1U);
	b[16U] = 2U;
...
static EFI_STATUS close_protocol(void *context, void *controller, const EFI_GUID *guid)
{
	...
	(void)guid;
	f->closes++;
	return f->closes == f->fail_close ? EFI_DEVICE_ERROR : EFI_SUCCESS;
}
			  ...
			  void *interface)
{
	...
	(void)interface;
	f->publishes++;
	return f->publishes == f->fail_publish ? EFI_DEVICE_ERROR : EFI_SUCCESS;
}
			    ...
			    void *interface)
{
	...
	(void)interface;
	f->unpublishes++;
	return f->unpublishes == f->fail_unpublish ? EFI_DEVICE_ERROR : EFI_SUCCESS;
}
...
static EFI_STATUS signal(void *context, void *event)
{
	...
	if (event == NULL)
		return EFI_INVALID_PARAMETER;
	f->signals++;
	if (close_on_signal != NULL) {
		struct cdk2_fat_file_protocol *file = close_on_signal;
		close_on_signal = NULL;
		reentrant_close_status = file->close(file);
	}
	return EFI_SUCCESS;
}
...
{
	(void)context;
	queued_function = function;
	queued_context = opaque;
	*cookie = opaque;
	return EFI_SUCCESS;
}
static void drain(void *context, void *cookie)
{
	(void)context;
	(void)cookie;
	queued_function(queued_context);
	queued_function = NULL;
}
static EFI_STATUS create_event(void *context, void(CDK2_MS_ABI *notify)(void *, void *),
			       void *opaque, void **event)
{
	unsigned int id = event_count++;
	(void)context;
	event_notify[id] = notify;
	event_context[id] = opaque;
	*event = (void *)(UINTN)(id + 4U);
	return EFI_SUCCESS;
}
...
{
	if (!ok)
		fprintf(stderr, "FAT binding test: %s\n", message);
	return !ok;
}
...
int main(void)
{
	static const struct cdk2_fat_binding_ops ops = {
	    ...
	    signal,	   queue,	   drain,   create_event, close_event, wait_event};
	struct fixture f = {0};
	struct cdk2_unicode_collation collation = {
	    .stri_coll = collate, .str_upr = upper, .str_to_fat = to_fat};
	...
	f.have_disk2 = 1U;
	failures +=
	    expect(cdk2_fat_complete_io(&binding, &token, EFI_DEVICE_ERROR) == EFI_SUCCESS &&
		       token.transaction_status == EFI_DEVICE_ERROR && f.signals == 1U,
		   "revision-2 completion did not publish status before signal");
	for (id = 0U; id < 2U; id++) {
		format(&f, id);
		f.media[id] = (struct cdk2_block_media){.media_id = id + 1U,
							.media_present = 1U,
							.block_size = 512U,
							.last_block = 4095U};
			   ...
			   "independent controllers did not publish independent volumes");
	{
		...
		failures += expect(
		    binding.mounts->simple_fs->protocol.open_volume(
		...
		disk_notify(disk_token->event, disk_context);
		failures += expect(root->close(root) == EFI_SUCCESS &&
				       async.status == EFI_SUCCESS && f.signals == 2U,
				   "DiskIo2 completion did not release handle residency");
	}
	{
		struct cdk2_fat_file_protocol *root, *file;
		struct cdk2_fat_file_io_token token = {(void *)9, EFI_NOT_READY, 0U, NULL};
		unsigned int guard = 0U, calls = disk2_calls;
		failures +=
		    expect(binding.mounts->simple_fs->protocol.open_volume(
			       &binding.mounts->simple_fs->protocol, &root) == EFI_SUCCESS &&
			       root->open_ex(root, &file, L"ASYNC.TXT", 1U, 0U, &token) ==
				   EFI_SUCCESS &&
		...
		failures += expect(
		    token.status == EFI_SUCCESS && file != NULL && queued_function == NULL,
	...
	}
	{
		...
		(void)file->read_ex(file, &one);
		(void)file->read_ex(file, &two);
		disk_token->transaction_status = EFI_DEVICE_ERROR;
		disk_notify(disk_token->event, disk_context);
		failures += expect(one.status == EFI_DEVICE_ERROR &&
				       two.status == EFI_NOT_READY && disk2_offset == 16896U,
	...
	}
	{
		...
		(void)binding.mounts->simple_fs->protocol.open_volume(
		    &binding.mounts->simple_fs->protocol, &root);
		close_on_signal = root;
		reentrant_close_status = EFI_NOT_READY;
		(void)root->flush_ex(root, &token);
		disk_token->transaction_status = EFI_SUCCESS;
		disk_notify(disk_token->event, disk_context);
		failures +=
		    expect(token.status == EFI_SUCCESS && reentrant_close_status == EFI_SUCCESS,
	...
	}
	{
		...
		info.create = (struct test_time){
		    .year = 2026U, .month = 13U, .day = 1U, .timezone = 0x07ff};
		info.modify = info.create;
		info.file_name[0] = L'X';
		info.file_name[1] = 0U;
		...
		info.create.month = 1U;
		info.modify.month = 1U;
		info.attribute = 1ULL << 40;
		failures += expect(
		    root->set_info(root, (EFI_GUID *)&cdk2_fat_file_info_guid, sizeof(info),
		...
		if (guard != MAP_FAILED) {
			...
			*last = L'Z';
			(void)mprotect(guard + page_size, (size_t)page_size, PROT_NONE);
			failures += expect(
			    root->set_info(root, (EFI_GUID *)&cdk2_fat_volume_label_info_guid,
	...
	}
	{
		struct cdk2_fat_file_protocol *root, *file;
		UINT8 output[600];
		struct cdk2_fat_file_io_token token = {(void *)9, EFI_NOT_READY, sizeof(output),
						       output};
		failures += expect(
		    binding.mounts->simple_fs->protocol.open_volume(
			&binding.mounts->simple_fs->protocol, &root) == EFI_SUCCESS &&
		...
		disk_notify(disk_token->event, disk_context);
		failures +=
		    expect(token.status == EFI_SUCCESS && token.buffer_size == 600U &&
	...
	}
	{
		...
		struct cdk2_fat_file_io_token one = {(void *)9, EFI_NOT_READY, sizeof(first),
						     first};
		struct cdk2_fat_file_io_token two = {(void *)8, EFI_NOT_READY, sizeof(second),
						     second};
		unsigned int calls = disk2_calls;
		(void)binding.mounts->simple_fs->protocol.open_volume(
		    &binding.mounts->simple_fs->protocol, &root);
		(void)root->open(root, &file, L"ASYNC.TXT", 1U, 0U);
		failures += expect(file->read_ex(file, &one) == EFI_SUCCESS &&
				       file->read_ex(file, &two) == EFI_SUCCESS &&
				       disk2_calls == calls + 1U,
				   "same-handle async requests were submitted out of order");
		memcpy(disk2_buffer, f.image[1] + disk2_offset, disk2_size);
		disk_token->transaction_status = EFI_SUCCESS;
		disk_notify(disk_token->event, disk_context);
		failures +=
		    expect(one.status == EFI_SUCCESS && two.status == EFI_NOT_READY &&
		...
		failures += expect(two.status == EFI_NOT_READY && disk2_offset == 17408U &&
				       disk2_size == 88U,
				   "serialized request did not cross its FAT extent");
		memcpy(disk2_buffer, f.image[1] + disk2_offset, disk2_size);
		disk_token->transaction_status = EFI_SUCCESS;
		disk_notify(disk_token->event, disk_context);
	...
	}
	{
		struct cdk2_fat_file_protocol *root, *file;
		UINT8 input[600];
		struct cdk2_fat_file_io_token token = {(void *)9, EFI_NOT_READY, sizeof(input),
						       input};
		...
		disk_notify(disk_token->event, disk_context);
		failures +=
		    expect(token.status == EFI_DEVICE_ERROR && token.buffer_size == 0U,
	...
	}
	{
		...
		(void)root->open(root, &file, L"ASYNC.TXT", 3U, 0U);
		{
			...
			if (write_status != EFI_SUCCESS)
				token.status = write_status;
		}
		while (token.status == EFI_NOT_READY && guard++ < 512U)
			complete_disk(EFI_SUCCESS, 0);
		failures +=
		    expect(token.status == EFI_SUCCESS && token.buffer_size == sizeof(input) &&
	...
	}
	{
		...
		(void)binding.mounts->simple_fs->protocol.open_volume(
		    &binding.mounts->simple_fs->protocol, &root);
		(void)root->open(root, &file, L"ASYNC.TXT", 3U, 0U);
		(void)file->write_ex(file, &one);
		(void)file->write_ex(file, &two);
		while ((one.status == EFI_NOT_READY || two.status == EFI_NOT_READY) &&
		       guard++ < 2048U)
			complete_disk(EFI_SUCCESS, 0);
		(void)file->get_position(file, &position);
		failures += expect(
		    one.status == EFI_SUCCESS && two.status == EFI_SUCCESS && position == 1400U,
	...
	}
	{
		...
		while ((one.status == EFI_NOT_READY || two.status == EFI_NOT_READY) &&
			...
			if (!failed && disk2_operation == 1U) {
				failed = 1;
				complete_disk(EFI_DEVICE_ERROR, 0);
			} else
				complete_disk(EFI_SUCCESS, 0);
		}
		(void)file->get_position(file, &position);
		failures +=
		    expect(one.status == EFI_DEVICE_ERROR && two.status == EFI_SUCCESS &&
			       position == 100U,
	...
	}
	{
		...
		struct cdk2_fat_file_io_token token = {(void *)9, EFI_NOT_READY, sizeof(input),
						       input};
		unsigned int guard = 0U, first_write;
		int rollback_failure = 0;
		memcpy(before, f.image[1], sizeof(before));
		...
		while (token.status == EFI_NOT_READY && guard++ < 1024U) {
			...
			else if (disk2_operation == 1U && disk2_writes == first_write + 3U) {
				rollback_failure = 1;
		...
		failures += expect(
		    token.status == EFI_DEVICE_ERROR && token.buffer_size == 0U &&
			rollback_failure && memcmp(before, f.image[1], sizeof(before)) == 0,
		    "rollback failure replaced the first error or stopped reverse recovery");
		(void)file->close(file);
		(void)root->close(root);
	}
	failures += expect(cdk2_fat_binding_start(&binding, (void *)1) == FAT_ALREADY_STARTED,
			   "repeated Start was not idempotently rejected");
	first = binding.mounts;
	failures +=
	    expect(cdk2_fat_binding_open_handle(first) == EFI_SUCCESS &&
	...
	failures +=
	    expect(cdk2_fat_binding_stop(&binding, first->controller) == EFI_DEVICE_ERROR &&
		       binding.mounts == first,
		   "unpublish failure destroyed live volume ownership");
	f.fail_unpublish = 0U;
	failures += expect(cdk2_fat_binding_stop(&binding, first->controller) == EFI_SUCCESS &&
			       cdk2_fat_binding_stop(&binding, (void *)1) == EFI_SUCCESS &&
	...
	failures += expect(cdk2_fat_binding_start(&binding, (void *)1) == EFI_DEVICE_ERROR &&
			       binding.mounts == NULL && f.allocations == f.releases,
	...
	failures += expect(cdk2_fat_binding_stop(&binding, (void *)1) == EFI_SUCCESS &&
			       binding.mounts == NULL,
			   "Stop rollback state was not retryable");
	f.have_disk2 = 0U;
	failures += expect(cdk2_fat_binding_start(&binding, (void *)1) == EFI_SUCCESS,
			   "DiskIo-only parent was rejected");
	{
		...
		failures +=
		    expect(binding.mounts->simple_fs->protocol.open_volume(
			       &binding.mounts->simple_fs->protocol, &root) == EFI_SUCCESS &&
			       root->revision == 0x00010000ULL &&
			       root->flush_ex(root, &token) == EFI_UNSUPPORTED,
			   "DiskIo-only parent incorrectly advertised revision 2");
		(void)root->close(root);
	}
	failures += expect(cdk2_fat_binding_stop(&binding, (void *)1) == EFI_SUCCESS,
