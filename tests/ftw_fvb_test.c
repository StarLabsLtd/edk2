...
static EFI_STATUS read(void *c, void *v, UINT64 lba, void *data, UINTN bytes)
{
	...
	if (lba > 1 || bytes != BS)
		return EFI_INVALID_PARAMETER;
	memcpy(data, ((struct volume *)v)->data[lba], bytes);
	return 0;
}
static EFI_STATUS write(void *c, void *v, UINT64 lba, const void *data, UINTN bytes)
{
	...
	if (lba > 1 || bytes != BS)
		return EFI_INVALID_PARAMETER;
	memcpy(((struct volume *)v)->data[lba], data, bytes);
	return 0;
}
...
static BOOLEAN is_boot(void *c, void *v)
{
	(void)c;
	return ((struct volume *)v)->boot;
}
...
{
	struct fixture f = { 0 };
	struct volume work = { 0 }, spare = { 0 };
	struct volume normal = { 0 }, boot = { .boot = TRUE };
	UINT8 scratch[BS * 2];
	UINT8 update[4] = { 1, 2, 3, 4 };
	EFI_GUID id = { .data1 = 1 };
	struct cdk2_ftw_fvb a = { .ops = &ops, .context = &f, .working_volume = &work,
		       .spare_volume = &spare, .working_lba = 0, .spare_lba = 0, .block_count = 2
	};
	memset(normal.data, 0x33, sizeof(normal.data));
	memset(boot.data, 0x44, sizeof(boot.data));
