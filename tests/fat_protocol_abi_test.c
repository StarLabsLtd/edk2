...
{
	if (!ok) fprintf(stderr, "FAT protocol ABI test: %s\n", message);
	return !ok;
}
...
int main(void)
{
	...
	failures += expect(volume.protocol.open_volume(NULL, &file) ==
		EFI_INVALID_PARAMETER && file == (void *)1,
		"NULL protocol fault path changed caller ownership");
	failures += expect(volume.protocol.open_volume(&volume.protocol, NULL) ==
		EFI_INVALID_PARAMETER, "NULL result fault was not rejected");
