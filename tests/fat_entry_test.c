...
static unsigned int protocols;
static EFI_STATUS failure;
static EFI_STATUS CDK2_MS_ABI allocate(UINT32 t, UINTN s, void **p)
{
	(void)t;
	*p = malloc(s);
	return *p == NULL ? EFI_OUT_OF_RESOURCES : EFI_SUCCESS;
}
static EFI_STATUS CDK2_MS_ABI release(void *p)
{
	free(p);
	return EFI_SUCCESS;
}
...
int main(void)
{
	...
	if (cdk2_fat_entry(NULL, &system) != EFI_INVALID_PARAMETER)
		return 1;
