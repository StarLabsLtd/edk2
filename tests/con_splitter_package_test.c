...
{
	static const uint8_t guid[16] = { 0xec, 0xdc, 0x8e, 0x40, 0x6d, 0xcf,
		0x7c, 0x47, 0xa5, 0xa8, 0xb4, 0x84, 0x4e, 0x3d, 0xe2, 0x81 };
	...
	if (argc != 3 || (file = fopen(argv[1], "rb")) == NULL)
		return 1;
	if (fseek(file, 0, SEEK_END) != 0 || (length = ftell(file)) != 0xb04e ||
	...
	if ((file = fopen(argv[2], "rb")) == NULL || fseek(file, 0, SEEK_END) != 0 ||
	    (pe_length = ftell(file)) <= 0 || pe_length > 0xb000 ||
	...
	if (memcmp(data, guid, sizeof(guid)) != 0 || (sum & 0xffU) != 0U ||
	    data[17] != 0xaa || data[18] != 7 || size24(data + 20) != 0xb04e ||
	    data[23] != 0xf8)
		goto bad;
	for (index = 0; index < sizeof(types); index++) {
		...
		if (index == 0 && (section_size != 0xb004 ||
		    memcmp(data + offset + 4, pe, (size_t)pe_length) != 0))
			goto bad;
		if (index == 0)
			for (size_t pad = (size_t)pe_length; pad < 0xb000; pad++)
				if (data[offset + 4 + pad] != 0)
					goto bad;
		if (index == 1 && (section_size != 0x22 ||
		    memcmp(data + offset + 4,
		    "C\0o\0n\0S\0p\0l\0i\0t\0t\0e\0r\0D\0x\0e\0\0\0", 30) != 0))
			goto bad;
