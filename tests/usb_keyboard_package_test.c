...
{
	static const uint8_t guid[16] = { 0xcf, 0x62, 0x2e, 0x2d, 0xcf, 0x9e,
		0xb7, 0x43, 0x82, 0x19, 0x94, 0xe7, 0xfc, 0x71, 0x3d, 0xfe };
	static const uint8_t types[] = { 0x10, 0x15, 0x14 };
	static const uint8_t ui[] = { 'U', 0, 's', 0, 'b', 0, 'K', 0, 'b', 0,
		'D', 0, 'x', 0, 'e', 0, 0, 0 };
	...
	if (argc != 2 || (file = fopen(argv[1], "rb")) == NULL)
		return 1;
	if (fseek(file, 0, SEEK_END) != 0 || (length = ftell(file)) != 0x9042L ||
	...
	if (memcmp(data, guid, sizeof(guid)) != 0 || (sum & 0xffU) != 0U ||
	    data[17] != 0xaaU || data[18] != 7U || size24(data + 20) != 0x9042U ||
	    data[23] != 0xf8U) {
		free(data);
		return 1;
	}
