...
{
	UINT8 workspace[512];
	struct cdk2_ftw_journal in = { 0 }, out;
	unsigned int index;
	const struct cdk2_ftw_pi_work_header *wh;
	...
	in.magic = CDK2_FTW_MAGIC;
	in.caller_id.data1 = 0x1234;
	in.write_count = 1;
	in.private_size = 2;
	in.phase = CDK2_FTW_SPARE_COMPLETE;
	in.records[0] = (struct cdk2_ftw_record) {
		.lba = 7, .offset = 8, .length = 9,
	...
	h = (const void *)(workspace + sizeof(*wh));
	r = (const void *)(h + 1);
	...
	for (index = 1; index < 9; index++) {
		in.caller_id.data1 = 0x2000 + index;
		in.phase = CDK2_FTW_SPARE_COMPLETE;
	...
	workspace[0] ^= 1;
	assert(cdk2_ftw_pi_decode(workspace, sizeof(workspace), &out) == EFI_VOLUME_CORRUPTED);
	return 0;
}
