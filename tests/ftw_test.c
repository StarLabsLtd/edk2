...
struct media {
	...
	unsigned int operation, fail_at;
};
static EFI_STATUS fault(struct media *m)
{
	m->operation++;
	return m->fail_at == m->operation ? EFI_DEVICE_ERROR : EFI_SUCCESS;
}
...
{
	struct media *m = p;
	EFI_STATUS s = fault(m);
	if (EFI_ERROR(s))
		return s;
	if (!m->journal_exists) {
		s = cdk2_ftw_pi_initialize(m->workspace, sizeof(m->workspace));
		if (EFI_ERROR(s))
			return s;
	}
...
static EFI_STATUS target_erase(void *p, UINT64 lba)
{
	...
	if (lba > 1)
		return EFI_INVALID_PARAMETER;
	memset(m->target[lba], 0xff, BLOCK);
	return 0;
}
...
{
	return (struct cdk2_ftw) {
		&ops, m, BLOCK, scratch, { 0 }, 0
	};
}
...
static void crash_matrix(void)
{
	...
	for (cut = 1; cut <= 9; cut++) {
		struct media m = { 0 };
		UINT8 scratch[BLOCK], update[5] = { 3, 4, 5, 6, 7 };
		EFI_GUID id = { .data1 = cut };
		struct cdk2_ftw ftw = make_ftw(&m, scratch);
...
{
	struct media m = { 0 };
	UINT8 scratch[BLOCK], update[5] = { 9, 8, 7, 6, 5 };
	EFI_GUID id = { .data1 = 7 }, got;
	struct cdk2_ftw ftw = make_ftw(&m, scratch);
	UINT64 lba;
	UINTN offset, length, private_size;
	BOOLEAN complete;
	UINT8 private = 0xaa;
	memset(m.target[0], 1, BLOCK);
	memset(m.target[1], 2, BLOCK);
	assert(cdk2_ftw_initialize(&ftw) == EFI_SUCCESS);
