...
struct hob_list { struct guid_hob hob; SMMSTORE_INFO info; struct hob_header end; };
...
{ boot.slots_before_locate_protocol[(offset - 24U) / sizeof(void *)] = function; }
static EFI_STATUS CDK2_MS_ABI allocate_pool(UINT32 type, UINTN size, void **buffer)
{
	(void)type; allocations++;
	if (allocations == allocation_fail_at) return EFI_OUT_OF_RESOURCES;
	if (allocations == 1U && size == sizeof(scratch)) *buffer = scratch;
	else if (allocations == 2U && size == sizeof(workspace)) *buffer = workspace;
	else return EFI_OUT_OF_RESOURCES;
	return EFI_SUCCESS;
}
static EFI_STATUS CDK2_MS_ABI free_pool(void *buffer)
{ assert(buffer != NULL); frees++; return EFI_SUCCESS; }
...
{ assert(self == &fvb && lba < 8U && offset + *bytes <= BLOCK_SIZE);
	...
	memcpy(buffer, media[lba] + offset, *bytes); return EFI_SUCCESS; }
static EFI_STATUS CDK2_MS_ABI write_block(void *self, UINT64 lba, UINTN offset,
	UINTN *bytes, const void *buffer)
{ assert(self == &fvb && lba < 8U && offset + *bytes <= BLOCK_SIZE);
	memcpy(media[lba] + offset, buffer, *bytes); return EFI_SUCCESS; }
static EFI_STATUS CDK2_MS_ABI erase_blocks(void *self, ...)
{
	va_list args; UINT64 lba; UINTN count;
	assert(self == &fvb); va_start(args, self); lba = va_arg(args, UINT64);
	count = va_arg(args, UINTN); va_end(args); assert(lba + count <= 8U);
	memset(media[lba], 0xff, count * BLOCK_SIZE); return EFI_SUCCESS;
}
...
static void prepare(void)
{
	...
	memset(&boot,0,sizeof(boot)); memset(&system,0,sizeof(system));
	memset(&hobs,0,sizeof(hobs)); memset(media,0xff,sizeof(media));
	memset(&fvb,0,sizeof(fvb)); cdk2_ftw_entry_reset_for_test();
	runtime_status=EFI_SUCCESS;handles_status=EFI_SUCCESS;event_status=EFI_SUCCESS;
	register_status=EFI_SUCCESS;read_status=EFI_SUCCESS;install_status=EFI_SUCCESS;
	allocations=allocation_fail_at=frees=closes=installs=0;notification=NULL;
	set_slot(64U,allocate_pool);set_slot(72U,free_pool);set_slot(80U,create_event);
	set_slot(112U,close_event);set_slot(128U,install_protocol);
	set_slot(152U,handle_protocol);set_slot(168U,register_notify);
	set_slot(312U,locate_handles);boot.locate_protocol=locate_protocol;
	fvb.get_physical_address=get_address;fvb.get_block_size=get_block;
	fvb.read=read_block;fvb.write=write_block;fvb.erase_blocks=erase_blocks;
	hobs.hob.header.type=4U;hobs.hob.header.length=sizeof(hobs.hob)+sizeof(hobs.info);
	hobs.hob.name=info_guid;hobs.info.num_blocks=8U;hobs.info.block_size=BLOCK_SIZE;
	hobs.info.mmio_address=0xff800000ULL;hobs.end.type=0xffffU;
	hobs.end.length=sizeof(hobs.end);configuration.guid=hob_guid;
	configuration.table=&hobs;system.boot=&boot;system.table_count=1U;
	system.tables=&configuration;
}

int main(void)
{
	prepare();system.table_count=0;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_NOT_FOUND);
	prepare();runtime_status=EFI_NOT_FOUND;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_NOT_READY&&notification==NULL);
	prepare();handles_status=EFI_NOT_FOUND;register_status=EFI_DEVICE_ERROR;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_DEVICE_ERROR&&closes==1U);
	prepare();allocation_fail_at=1U;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_OUT_OF_RESOURCES&&frees==1U);
	prepare();allocation_fail_at=2U;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_OUT_OF_RESOURCES&&frees==2U);
	prepare();read_status=EFI_DEVICE_ERROR;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_DEVICE_ERROR&&frees==3U);
	prepare();install_status=EFI_DEVICE_ERROR;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_DEVICE_ERROR&&frees==3U);
	prepare();handles_status=EFI_NOT_FOUND;
	assert(cdk2_ftw_entry((void *)1,&system)==EFI_SUCCESS&&notification!=NULL);
	handles_status=EFI_SUCCESS;notification((void *)0x55,NULL);
