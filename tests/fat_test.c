...
static INTN CDK2_MS_ABI latin_collate(struct cdk2_unicode_collation *self, CHAR16 *left,
				      CHAR16 *right)
{
	(void)self;
	while (latin_upper(*left) == latin_upper(*right) && *left != 0U) {
		left++;
		right++;
	}
	return latin_upper(*left) - latin_upper(*right);
}
static VOID CDK2_MS_ABI latin_str_upr(struct cdk2_unicode_collation *self, CHAR16 *text)
{
	(void)self;
	while (*text != 0U) {
		*text = latin_upper(*text);
		text++;
	}
}
static struct cdk2_unicode_collation latin_collation = {.stri_coll = latin_collate,
							.str_upr = latin_str_upr};
...
static uint64_t mutation_read(void *opaque, uint64_t offset, size_t size, void *buffer)
{
...
static uint64_t mutation_write(void *opaque, uint64_t offset, size_t size, const void *buffer)
{
	struct mutation_medium *medium = opaque;
	medium->writes++;
...
static void make_bpb(struct medium *medium, uint32_t sectors, uint16_t roots, uint16_t fat16,
		     uint32_t fat32, uint8_t cluster)
{
	memset(medium, 0, sizeof(*medium));
	write16(medium->boot + 11U, 512U);
	medium->boot[13U] = cluster;
	write16(medium->boot + 14U, fat32 == 0U ? 1U : 32U);
	medium->boot[16U] = 2U;
	write16(medium->boot + 17U, roots);
	write32(medium->boot + 32U, sectors);
	write16(medium->boot + 22U, fat16);
	write32(medium->boot + 36U, fat32);
	write32(medium->boot + 44U, 2U);
	medium->boot[510U] = 0x55U;
	medium->boot[511U] = 0xaaU;
	medium->size = (uint64_t)sectors * 512U;
}
static int expect(int condition, const char *message)
{
	if (!condition)
		fprintf(stderr, "FAT test: %s\n", message);
	return !condition;
}
...
static void lfn_record(uint8_t *record, uint8_t ordinal, uint8_t sum, const uint16_t *name)
{
	static const uint8_t positions[13] = {1U,  3U,	5U,  7U,  9U,  14U, 16U,
					      18U, 20U, 22U, 24U, 28U, 30U};
	unsigned int index;
	memset(record, 0xff, 32U);
	record[0] = ordinal;
	record[11U] = 0x0fU;
	record[12U] = 0U;
	record[13U] = sum;
	write16(record + 26U, 0U);
...
int main(void)
{
	...
	uint16_t long_name[13] = {'L', 'o', 'n', 'g', ' ', 'N', 'a',
				  'm', 'e', '.', 't', 'x', 't'};
	...
	failures +=
	    expect(cdk2_fat_probe(&volume, read_media, &medium, medium.size) == EFI_SUCCESS &&
	...
	failures +=
	    expect(cdk2_fat_probe(&volume, read_media, &medium, medium.size) == EFI_SUCCESS &&
		       volume.fat_type == CDK2_FAT32 &&
		       cdk2_fat_cluster_offset(&volume, 2U, &offset) == EFI_SUCCESS &&
		       offset == volume.data_offset,
		   "valid FAT32 geometry was not mapped");
	medium.boot[510U] = 0U;
	failures +=
	    expect(cdk2_fat_probe(&volume, read_media, &medium, medium.size) == EFI_UNSUPPORTED,
		   "missing boot signature was admitted");
	make_bpb(&medium, 200000U, 0U, 0U, 1000U, 3U);
	failures +=
	    expect(cdk2_fat_probe(&volume, read_media, &medium, medium.size) == EFI_UNSUPPORTED,
		   "non-power-of-two cluster geometry was admitted");
	make_bpb(&medium, 200000U, 0U, 0U, 1000U, 1U);
	medium.size--;
	failures += expect(cdk2_fat_probe(&volume, read_media, &medium, medium.size) ==
			       FAT_VOLUME_CORRUPTED,
			   "truncated medium was admitted");
	volume = (struct cdk2_fat_volume){.read = read_chain,
					  .context = &chain,
					  .media_size = 160U,
					  .fat_offset = 64U,
					  .data_offset = 128U,
	...
	write16(chain.fat + 4U, 3U);
	write16(chain.fat + 6U, 0xffffU);
	memcpy(chain.data, "ABCDEFGH", 8U);
	size = 6U;
	failures +=
	    expect(cdk2_fat_read_file(&volume, 2U, 8U, 1U, &size, output) == EFI_SUCCESS &&
		       size == 6U && memcmp(output, "BCDEFG", 6U) == 0,
	...
	volume.fat_type = CDK2_FAT12;
	{
		...
		int end;
		failures += expect(
		    cdk2_fat_next_cluster(&volume, 2U, &next, &end) == EFI_SUCCESS &&
			next == 3U && !end &&
		...
		failures += expect(cdk2_fat_next_cluster(&volume, 2U, &next, &end) ==
				       FAT_VOLUME_CORRUPTED,
				   "bad-cluster marker was admitted");
	}
	volume.fat_type = CDK2_FAT16;
	write16(chain.fat + 4U, 3U);
	write16(chain.fat + 6U, 0xffffU);
	...
	write16(chain.fat + 6U, 2U);
	size = 20U;
	failures += expect(cdk2_fat_read_file(&volume, 2U, 20U, 0U, &size, output) ==
			       FAT_VOLUME_CORRUPTED,
			   "cyclic file chain was not bounded");
	memcpy(records + 32U, "LONGNA~1TXT", 11U);
	records[32U + 11U] = 0x20U;
	write16(records + 32U + 26U, 2U);
	write32(records + 32U + 28U, 8U);
	lfn_record(records, 0x41U, checksum(records + 32U), long_name);
	failures += expect(
	    cdk2_fat_parse_directory_entry(records, 2U, &entry, &consumed) == EFI_SUCCESS &&
		consumed == 2U && entry.name[0] == 'L' && entry.name[12] == 't' &&
		entry.first_cluster == 2U && entry.size == 8U,
	    "valid long filename sequence was rejected");
	records[13U]++;
	failures += expect(cdk2_fat_parse_directory_entry(records, 2U, &entry, &consumed) ==
			       FAT_VOLUME_CORRUPTED,
			   "long filename checksum mismatch was admitted");
	memset(records, 0, sizeof(records));
	memcpy(records, "README  TXT", 11U);
	records[11U] = 0x20U;
	failures += expect(cdk2_fat_parse_directory_entry(records, 1U, &entry, &consumed) ==
				   EFI_SUCCESS &&
			       entry.name[6] == '.' && entry.name[9] == 'T',
			   "short filename fallback was not decoded");
	write16(records + 24U, (13U << 5) | 1U);
	failures += expect(cdk2_fat_parse_directory_entry(records, 1U, &entry, &consumed) ==
			       FAT_VOLUME_CORRUPTED,
			   "invalid FAT timestamp was admitted");
	write16(records + 24U, 0U);
	records[11U] = 0x80U;
	failures += expect(cdk2_fat_parse_directory_entry(records, 1U, &entry, &consumed) ==
			       FAT_VOLUME_CORRUPTED,
	...
	memcpy(chain.data, "DIR        ", 11U);
	chain.data[11U] = 0x10U;
	write16(chain.data + 20U, 0U);
	write16(chain.data + 26U, 4U);
	memcpy(chain.data + 64U, "NEST    TXT", 11U);
	chain.data[64U + 11U] = 0x20U;
	write16(chain.data + 64U + 26U, 5U);
	write32(chain.data + 64U + 28U, 5U);
	memcpy(chain.data + 96U, "HELLO", 5U);
	failures +=
	    expect(cdk2_fat_open_root(&volume, &root) == EFI_SUCCESS &&
		       cdk2_fat_open(&root,
				     (const uint16_t[]){'d', 'i', 'r', '/', 'n', 'e', 's', 't',
							'.', 't', 'x', 't', 0},
				     &file) == EFI_SUCCESS &&
		       !file.is_directory,
		   "case-insensitive nested path lookup failed");
	failures +=
	    expect(file.parent_directory_cluster == 4U && file.record_index == 0U &&
		       file.record_count == 1U,
		   "opened file did not retain its exact parent directory record range");
	chain.data[64U] = 0xc4U;
	failures += expect(cdk2_fat_open(&root,
					 (const uint16_t[]){'D', 'I', 'R', '/', 0xe4U, 'E', 'S',
							    'T', '.', 'T', 'X', 'T', 0},
					 &file) == EFI_SUCCESS,
			   "Unicode Collation was not used for non-ASCII lookup");
	chain.data[64U] = 'N';
	(void)cdk2_fat_open(
	    &root,
	    ...
	    &file);
	size = 0U;
	failures += expect(cdk2_fat_file_get_info(&file, &size, NULL) == EFI_BUFFER_TOO_SMALL &&
			       size == offsetof(struct cdk2_fat_file_info, name) + 18U,
			   "file info sizing contract is wrong");
	size = sizeof(info);
	failures +=
	    expect(cdk2_fat_file_get_info(&file, &size, &info) == EFI_SUCCESS &&
		   ...
		   "file info content or allocated PhysicalSize is wrong");
	{
		...
		failures += expect(
		    cdk2_fat_open(&root, (const uint16_t[]){'d', 'i', 'r', 0}, &dir) ==
			    EFI_SUCCESS &&
			cdk2_fat_open(&dir,
				      (const uint16_t[]){'\\', 'd', 'i', 'r', '\\', 'n', 'e',
							 's', 't', '.', 't', 'x', 't', 0},
				      &absolute) == EFI_SUCCESS &&
			absolute.entry.size == 5U,
		    "absolute path did not restart at the volume root");
		failures += expect(cdk2_fat_open(&dir, (const uint16_t[]){'.', 0}, &absolute) ==
					   EFI_SUCCESS &&
	...
	failures += expect(cdk2_fat_file_read(&file, &size, output) == EFI_SUCCESS &&
			       ...
			       cdk2_fat_file_get_position(&file, &offset) == EFI_SUCCESS &&
			       offset == 2U,
	...
	size = 1U;
	failures += expect(cdk2_fat_file_read(&root, &size, output) == EFI_BUFFER_TOO_SMALL &&
			       root.position == 0U && size > 1U,
	...
	failures += expect(
	    cdk2_fat_generate_short_name(&volume, 2U,
					 (const uint16_t[]){'L', 'o', 'n', 'g', ' ', 'f', 'i',
							    'l', 'e', '.', 't', 'x', 't', 0},
					 short_name) == EFI_SUCCESS &&
		memcmp(short_name, "LONGFI~1TXT", 11U) == 0,
	    "deterministic short-name generation failed");
	size = 0U;
	failures +=
	    expect(cdk2_fat_build_directory_records(
		       (const uint16_t[]){'L', 'o', 'n', 'g', ' ', 'f', 'i', 'l', 'e', '.', 't',
					  'x', 't', 0},
		       short_name, 0x20U, 5U, 5U, NULL, &size) == EFI_BUFFER_TOO_SMALL &&
		       size == 2U,
		   "LFN record sizing is wrong");
	failures +=
	    expect(cdk2_fat_build_directory_records(
		       (const uint16_t[]){'L', 'o', 'n', 'g', ' ', 'f', 'i', 'l', 'e', '.', 't',
					  'x', 't', 0},
		       short_name, 0x20U, 5U, 5U, built_records, &size) == EFI_SUCCESS &&
		       cdk2_fat_parse_directory_entry(built_records, size, &entry, &consumed) ==
			   EFI_SUCCESS &&
		       entry.name[0] == 'L' && entry.first_cluster == 5U && entry.size == 5U,
		   "generated LFN records did not round-trip");
	volume = (struct cdk2_fat_volume){.read = mutation_read,
					  .context = &mutation,
					  .media_size = 512U,
					  .fat_offset = 64U,
					  .data_offset = 128U,
	...
	write16(mutation.bytes + 96U + 4U, 0xffffU);
	{
		...
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 1U, 96U, changes,
						 &change_count) == EFI_SUCCESS &&
			       first == 2U && change_count == 3U &&
			       test_read16(mutation.bytes + 64U + 4U) == 3U &&
		...
		change_count = 8U;
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 96U, 0U, changes,
						 &change_count) == EFI_SUCCESS &&
			       first == 0U && test_read16(mutation.bytes + 64U + 4U) == 0U &&
		...
		first = 2U;
		mutation.writes = 0U;
		mutation.fail_write = 3U;
		change_count = 8U;
		failures += expect(cdk2_fat_resize_chain(&volume, &first, 1U, 64U, changes,
							 &change_count) == EFI_DEVICE_ERROR &&
		...
		change_count = 8U;
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 1U, 64U, changes,
						 &change_count) == CDK2_FAT_WRITE_PROTECTED,
			   "read-only volume admitted mutation");
		volume.read_only = 0U;
		volume.media_changed = 1U;
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 1U, 64U, changes,
						 &change_count) == CDK2_FAT_MEDIA_CHANGED,
			   "media-changed volume admitted mutation");
		volume.media_changed = 0U;
		volume.write_protected = 1U;
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 1U, 64U, changes,
						 &change_count) == CDK2_FAT_WRITE_PROTECTED,
	...
	write32(mutation.bytes + 1024U + 8U, 0x0fffffffU);
	{
		...
		size_t change_count = 8U;
		failures +=
		    expect(cdk2_fat_resize_chain(&volume, &first, 1U, 1024U, changes,
						 &change_count) == EFI_SUCCESS &&
		...
		change_count = 8U;
		failures += expect(cdk2_fat_resize_chain(&volume, &first, 1U, 1024U, changes,
							 &change_count) == EFI_DEVICE_ERROR &&
				       (test_read32(mutation.bytes + 1024U + 8U) &
	...
	write32(mutation.bytes + 1024U + 8U, 0x0fffffffU);
	{
		struct cdk2_fat_file created;
		size_t change_count = 8U;
		(void)cdk2_fat_open_root(&volume, &root);
		failures += expect(
		    cdk2_fat_create(&root,
				    (const uint16_t[]){'N', 'e', 'w', '.', 't', 'x', 't', 0},
				    0x20U, &created, changes, &change_count) == EFI_SUCCESS &&
			created.entry.name[0] == 'N' && created.entry.first_cluster == 0U,
		    "zero-length file creation failed");
		change_count = 8U;
		failures +=
		    expect(cdk2_fat_create(&root, (const uint16_t[]){'S', 'u', 'b', 0}, 0x10U,
					   &created, changes, &change_count) == EFI_SUCCESS &&
			       created.entry.first_cluster >= 3U,
			   "directory creation did not allocate and initialize a cluster");
	}
	memset(mutation.bytes, 0, sizeof(mutation.bytes));
	mutation.writes = 0U;
	    ...
	    (const uint8_t *)"CROSS~1 TXT", 0x20U, 4U, 7U, built_records, &size);
	{
		uint8_t rollback[64];
		uint64_t placed;
		size_t rollback_count = 2U;
		failures +=
		    expect(cdk2_fat_place_directory_records(&volume, 2U, built_records, 2U,
							    &placed, rollback,
							    &rollback_count) == EFI_SUCCESS &&
		...
		mutation.writes = 0U;
		mutation.fail_write = 2U;
		rollback_count = 2U;
		failures += expect(cdk2_fat_place_directory_records(
				       &volume, 2U, built_records, 2U, &placed, rollback,
				       &rollback_count) == EFI_DEVICE_ERROR &&
				       mutation.bytes[1536U + 15U * 32U] == 0U,
	...
	mutation.fail_write = 0U;
	{
		...
		failures +=
		    expect(cdk2_fat_place_directory_records(&volume, 2U, built_records, 2U,
							    ...
							    &rollback_count) == EFI_SUCCESS &&
			       placed == 16U &&
			       (test_read32(mutation.bytes + 1024U + 8U) & 0x0fffffffU) == 3U &&
			       mutation.bytes[2048U] == 0x41U,
	...
	mutation.fail_write = 3U;
	{
		...
		size_t rollback_count = 2U;
		failures +=
		    expect(cdk2_fat_place_directory_records(
			       &volume, 2U, built_records, 2U, &placed, rollback,
			       &rollback_count) == EFI_DEVICE_ERROR &&
	...
	failures += expect(
	    cdk2_fat_generate_short_name(&volume, 2U,
					 (const uint16_t[]){'L', 'o', 'n', 'g', ' ', 'f', 'i',
							    'l', 'e', '.', 't', 'x', 't', 0},
					 short_name) == EFI_SUCCESS &&
		memcmp(short_name, "LONGFI~2TXT", 11U) == 0,
	...
	write32(mutation.bytes + 1024U + 12U, 0x0fffffffU);
	{
		...
		size_t change_count = 8U;
		failures +=
		    expect(cdk2_fat_delete_entry(&volume, 2U, 0U, 1U, &first, 512U, 1, changes,
						 &change_count) == FAT_ACCESS_DENIED &&
	...
	write32(mutation.bytes + 1024U + 16U, 0x0fffffffU);
	{
		struct cdk2_fat_file directory = {.volume = &volume,
						  .parent_directory_cluster = 2U,
						  .record_index = 0U,
						  .record_count = 1U,
						  .is_directory = 1U};
		size_t change_count = 8U;
		directory.entry.first_cluster = 3U;
		failures += expect(cdk2_fat_file_delete(&directory, changes, &change_count) ==
					   EFI_SUCCESS &&
				       test_read32(mutation.bytes + 1024U + 12U) == 0U &&
	...
	mutation.fail_write = 2U;
	{
		...
		size_t change_count = 8U;
		failures +=
		    expect(cdk2_fat_resize_file(&volume, 2U, 0U, 1U, &first, 512U, 0U, changes,
						&change_count) == EFI_DEVICE_ERROR &&
			       first == 3U &&
			       (test_read32(mutation.bytes + 1024U + 12U) & 0x0fffffffU) ==
				   0x0fffffffU &&
	...
	mutation.writes = 0U;
	failures +=
	    expect(cdk2_fat_rename_entry(&volume, 2U, 0U, 1U,
					 (const uint16_t[]){'R', 'e', 'n', 'a', 'm', 'e', 'd',
							    '.', 'b', 'i', 'n', 0},
					 0x20U, 3U, 512U, &offset) == EFI_SUCCESS &&
		       offset == 1U && mutation.bytes[1536U] == 0xe5U &&
		       mutation.bytes[1536U + 32U] == 0x41U,
	...
	mutation.writes = mutation.flushes = 0U;
	size = 6U;
	failures += expect(
	    cdk2_fat_write_file(&volume, 2U, 1024U, 510U, &size, "ABCDEF") == EFI_SUCCESS &&
		size == 6U && memcmp(mutation.bytes + 1536U + 510U, "AB", 2U) == 0 &&
		memcmp(mutation.bytes + 2048U, "CDEF", 4U) == 0 && mutation.flushes == 1U,
	    "cross-cluster byte write or flush failed");
	mutation.writes = 0U;
	mutation.fail_write = 2U;
	size = 6U;
	failures += expect(cdk2_fat_write_file(&volume, 2U, 1024U, 510U, &size, "GHIJKL") ==
				   EFI_DEVICE_ERROR &&
			       size == 2U,
	...
	volume.write_protected = 1U;
	size = 1U;
	failures += expect(cdk2_fat_write_file(&volume, 2U, 1024U, 0U, &size, "X") ==
			       CDK2_FAT_WRITE_PROTECTED,
	...
	write32(mutation.bytes + 1024U + 20U, 0U);
	{
		...
		failures += expect(
		    cdk2_fat_get_volume_info(&volume, &volume_info) == EFI_SUCCESS &&
			volume_info.volume_size == 2048U && volume_info.free_space == 1024U &&
			volume_info.block_size == 512U && volume_info.label[0] == 'M' &&
			volume_info.label[2] == 'V',
		    "volume size/free-space/label information is wrong");
		failures += expect(
		    cdk2_fat_set_volume_label(
			&volume, (const uint16_t[]){'N', 'e', 'w', ' ', 'L', 'a', 'b', 'e', 'l',
						    0}) == EFI_SUCCESS &&
	...
	file = (struct cdk2_fat_file){.volume = &volume,
				      ...
				      .record_count = 1U};
	mutation.flush_status = EFI_SUCCESS;
	failures += expect(cdk2_fat_update_metadata(&file, 0x21U, 0x5821U, 0x1000U, 0x5822U,
						    0x2000U) == EFI_SUCCESS &&
			       mutation.bytes[1536U + 32U + 11U] == 0x21U &&
			       file.entry.write_date == 0x5822U,
			   "validated file metadata was not committed");
	mutation.flush_status = EFI_DEVICE_ERROR;
	failures += expect(cdk2_fat_update_metadata(&file, 0x22U, 0x5821U, 0x1000U, 0x5822U,
						    0x2000U) == EFI_DEVICE_ERROR &&
			       mutation.bytes[1536U + 32U + 11U] == 0x21U &&
			       file.entry.attributes == 0x21U,
			   "metadata flush failure did not restore disk and handle state");
	mutation.flush_status = EFI_SUCCESS;
	failures +=
	    expect(cdk2_fat_file_rename(&file, (const uint16_t[]){'N', 'e', 'w', ' ', 'M', 'e',
								  't', 'a', '.', 't', 'x', 't',
								  0}) == EFI_SUCCESS &&
		       file.entry.name[0] == 'N' && file.record_index == 2U &&
