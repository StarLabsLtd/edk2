...
int main(int argc, char **argv)
{
	...
	uint8_t *data, *pe; FILE *file; long length, pe_length; size_t offset = 24;
	unsigned int sum = 0;
	...
	if ((file = fopen(argv[2], "rb")) == NULL || fseek(file, 0, SEEK_END) != 0 ||
		...
		free(data); if (file != NULL) fclose(file); return 1;
	}
	fclose(file);
	for (unsigned int index = 0; index < 24; index++) sum += data[index];
	sum -= data[17] + data[23];
	if (memcmp(data, guid, 16) != 0 || (sum & 0xffU) != 0U || data[17] != 0xaa ||
	    data[18] != 7 || size24(data + 20) != 0x9042 || data[23] != 0xf8) goto bad;
	for (unsigned int index = 0; index < sizeof(types); index++) {
		...
		if (index == 0 && (section_size != 0x9004 || data[offset + 4] != 'M' ||
		    ...
		    (size_t)pe_length) != 0)) goto bad;
		if (index == 0)
			for (size_t pad = (size_t)pe_length; pad < 0x9000; pad++)
				if (data[offset + 4 + pad] != 0x00) goto bad;
		if (index == 1 && (section_size != 0x18 || memcmp(data + offset + 4,
		    "A\0t\0a\0B\0u\0s\0D\0x\0e\0\0\0", 20) != 0)) goto bad;
		if (index == 2 && (section_size != 0x0e || memcmp(data + offset + 4,
		    "\0\x00\x31\0.\0\x30\0\0\0", 10) != 0)) goto bad;
		offset = (end + 3U) & ~(size_t)3U;
		if (offset > (size_t)length) offset = end;
	}
	free(pe); free(data); return offset == (size_t)length ? 0 : 1;
bad:
	free(pe); free(data); return 1;
}
