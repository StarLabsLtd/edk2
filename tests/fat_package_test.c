...
int main(int argc, char **argv)
{
	...
	uint8_t *data;
	long length;
	uint32_t offset, section_size;
	unsigned int section = 0;
	int ui_ok = 0, version_ok = 0;
	...
	if (fread(data, 1U, (size_t)length, file) != (size_t)length || fclose(file) != 0 ||
	    ...
	    size24(data + 20) != (uint32_t)length)
		return 1;
	for (offset = 24U; offset + 4U <= (uint32_t)length && section < 3U;) {
		...
		if (section_size < 4U || section_size > (uint32_t)length - offset ||
		    data[offset + 3U] != order[section])
			return 1;
		if (section == 1U && section_size >= 12U && data[offset + 4U] == 'F' &&
		    data[offset + 6U] == 'a' && data[offset + 8U] == 't' &&
		    data[offset + 10U] == 0U)
			ui_ok = 1;
		if (section == 2U && section_size >= 14U && data[offset + 6U] == '1' &&
		    data[offset + 8U] == '.' && data[offset + 10U] == '0' &&
		    data[offset + 12U] == 0U)
			version_ok = 1;
		section++;
		offset = (offset + section_size + 3U) & ~3U;
	}
	while (offset < (uint32_t)length && data[offset] == 0xffU)
		offset++;
	free(data);
	return section == 3U && ui_ok && version_ok && offset >= (uint32_t)length ? 0 : 1;
}
