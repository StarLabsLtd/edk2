...
{
	static const uint8_t guid[16] = { 0x91, 0x0e, 0xf5, 0xb7, 0x59, 0xa7,
		0x2c, 0x41, 0xad, 0xe4, 0xdc, 0xd0, 0x3e, 0x7f, 0x7c, 0x28 };
	static const uint8_t types[] = { 0x10, 0x15, 0x14 };
	static const uint8_t ui[] = { 'X', 0, 'h', 0, 'c', 0, 'i', 0, 'D', 0,
		'x', 0, 'e', 0, 0, 0 };
	...
	if (argc != 2 || (file = fopen(argv[1], "rb")) == NULL)
		return 1;
	if (fseek(file, 0, SEEK_END) != 0 || (length = ftell(file)) != 0x1103eL ||
	...
	if (memcmp(data, guid, sizeof(guid)) != 0 || (sum & 0xffU) != 0U ||
	    data[17] != 0xaaU || data[18] != 7U || size24(data + 20) != 0x1103eU ||
	    data[23] != 0xf8U) {
		free(data);
		return 1;
	}
	for (unsigned int index = 0; index < sizeof(types); index++) {
		...
		if ((index == 0U && (section_size != 0x11004U ||
		    data[offset + 4U] != 'M' || data[offset + 5U] != 'Z')) ||
		    (index == 1U && (section_size != 0x14U ||
